...
se.entry_date>= 
  (
    select date_sub(max(entry_date),interval 4 month) from stock_entries
 ) ;
...
select * from products where product_name= "Blue Device" ;

select * from reorders where reorder_id= 4 ;

select * from stock_entries where product_id=34 order by entry_date desc ;
